...
class NotaDeOutrosFormatosDisponiveisPanel extends StatefulWidget {
  final TextEditingController etiqueta,
      primeiroIndicador,
      segundoIndicador,
      $a_outroFormatoDisponivel,
      $b_fonte,
      $c_condicoes,
      $d_numeroDeOrdem,
      $u_URI,
  ...
  const NotaDeOutrosFormatosDisponiveisPanel({
    super.key,
    required this.etiqueta,
    required this.primeiroIndicador,
    required this.segundoIndicador,
    required this.$a_outroFormatoDisponivel,
    required this.$b_fonte,
    required this.$c_condicoes,
    required this.$d_numeroDeOrdem,
    required this.$u_URI,
  ...
  @override
  State<NotaDeOutrosFormatosDisponiveisPanel> createState() =>
      _NotaDeOutrosFormatosDisponiveisPanelState();
}

class _NotaDeOutrosFormatosDisponiveisPanelState
    extends State<NotaDeOutrosFormatosDisponiveisPanel> {
  bool value = false;
      ...
      children: [
        ExpansionPanel(
          ...
          headerBuilder: (context, isExpanded) {
            return ListTile(
              title: Text(
                '(530) - Nota de outros formatos disponpiveis',
          ...
          body: Padding(
            ...
            child: Column(
              children: [
                ...
                const SizedBox(height: 16.0),
                CustomTextFormFieldWidget(
                  controller: widget.$a_outroFormatoDisponivel,
                  titulo: '\$a - Outro formato disponível',
                ),
                const SizedBox(height: 16.0),
                CustomTextFormFieldWidget(
                  controller: widget.$b_fonte,
                  titulo: '\$b - Fonte',
                ),
                const SizedBox(height: 16.0),
                CustomTextFormFieldWidget(
                  controller: widget.$c_condicoes,
                  titulo: '\$c - Condições',
                ),
                const SizedBox(height: 16.0),
                CustomTextFormFieldWidget(
                  controller: widget.$d_numeroDeOrdem,
                  titulo: '\$d - Número de ordem',
                ),
                const SizedBox(height: 16.0),
                CustomTextFormFieldWidget(
